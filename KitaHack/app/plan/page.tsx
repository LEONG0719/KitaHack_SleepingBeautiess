...
import { ArrowRight, ArrowLeft, CalendarDays, CalendarRange } from 'lucide-react';
...
export default function PlanPage() {
  ...
  const [preferences, setPreferences] = useState<UserPreferences>({
    ...
    budgetPerMeal: 15,
    goals: 'balanced',
    cookingPreference: 'cook_dinner_only',
  });
        ...
        <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 sm:p-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
              ...
              <section>
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  <button
                    ...
                    className={`p-4 rounded-lg border-2 transition-all text-left flex items-center gap-3 ${planType === 'daily'
                        ? 'border-emerald-600 bg-emerald-50'
                        : 'border-gray-200 hover:border-gray-300'
                      }`}
                  >
                    <CalendarDays className={`w-6 h-6 ${planType === 'daily' ? 'text-emerald-600' : 'text-gray-400'}`} />
                    <div>
                      <div className={`text-sm font-bold ${planType === 'daily' ? 'text-emerald-700' : 'text-gray-900'}`}>Daily Plan</div>
                      <div className="text-xs text-gray-500">Generate a single day meal plan</div>
                    </div>
                  </button>
                  <button
                    type="button"
                    onClick={() => setPlanType('weekly')}
                    className={`p-4 rounded-lg border-2 transition-all text-left flex items-center gap-3 ${planType === 'weekly'
                        ? 'border-emerald-600 bg-emerald-50'
                        : 'border-gray-200 hover:border-gray-300'
                      }`}
                  >
                    <CalendarRange className={`w-6 h-6 ${planType === 'weekly' ? 'text-emerald-600' : 'text-gray-400'}`} />
                    <div>
                      <div className={`text-sm font-bold ${planType === 'weekly' ? 'text-emerald-700' : 'text-gray-900'}`}>Weekly Plan</div>
                      <div className="text-xs text-gray-500">7-day hybrid plan with smart grocery list</div>
                    </div>
                  </button>
                </div>
              </section>

              {/* 🔥 REMOVED THE CONDITION: Cooking Preference now shows for BOTH Daily and Weekly */}
              <section>
                <h2 className="text-xl font-bold text-gray-900 mb-2">
                  Cooking Preference
                </h2>
                <p className="text-sm text-gray-500 mb-4">
                  Choose which meals you want to cook vs. buy from outside
                </p>
                <div className="grid sm:grid-cols-2 gap-3">
                  {[
                    { value: 'cook_dinner_only', label: 'Cook Dinner Only', desc: 'Buy Breakfast & Lunch, Cook Dinner' },
                    { value: 'cook_breakfast_dinner', label: 'Cook Breakfast & Dinner', desc: 'Buy Lunch only' },
                    { value: 'cook_all', label: 'Cook All Meals', desc: 'Home Chef mode for every meal' },
                    { value: 'buy_all', label: 'Buy All Meals', desc: 'City Survivor — eat out for everything' },
                  ].map((option) => (
                    <label
                      key={option.value}
                      className={`flex items-start space-x-3 p-3 border-2 rounded-lg cursor-pointer transition-all ${preferences.cookingPreference === option.value
                          ? 'border-emerald-600 bg-emerald-50'
                          : 'border-gray-200 hover:border-gray-300'
                        }`}
                    >
                      <input
                        ...
                        value={option.value}
                        checked={preferences.cookingPreference === option.value}
                        onChange={(e) =>
                          setPreferences((prev) => ({
                            ...prev,
                            cookingPreference: e.target.value as any,
                          }))
                        }
              ...
              {/* ... (Basic Info, Dietary Rules, Food Prefs, Budget sections remain the same) ... */}
              <section>
                <h2 className="text-xl font-bold text-gray-900 mb-4">
                  Basic Information
                </h2>
              ...
              <div className="flex gap-4 pt-6 border-t">
                ...
                >
                  {planType === 'weekly' ? 'Generate 7-Day Plan' : 'Generate My Meal Plan'}
